...
router = APIRouter(dependencies=[Depends(require_current_admin)])
...
async def get_user_details(
    ...
    try:
        enroll_query = select(Enrollment.course_slug).where(Enrollment.user_id == user_uuid)
        enroll_result = await db.execute(enroll_query)
        enrolled_courses = [row.course_slug for row in enroll_result]
    except Exception as exc:
        raise HTTPException(status_code=500, detail="Failed to load enrollments") from exc
    ...
    for course_slug in enrolled_courses:
        try:
            ...
            raise HTTPException(status_code=500, detail="Failed to load user progress") from exc

    # Get activity details
    try:
        activity = await analytics_service.get_activity_details(user_id=user_uuid, db=db)
    except Exception as exc:
        raise HTTPException(status_code=500, detail="Failed to load user activity") from exc
